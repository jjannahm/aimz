...
import type { EventType, LiveMatchSnapshot, MatchEvent, MatchPhaseAction, PenaltyOutcome, SubstitutionReason } from '@/src/types/api';

type CardType = Extract<EventType, 'yellow_card' | 'red_card'>;
type EventGroup = 'goal' | 'cards' | 'substitution';
/** What the Goal tab is logging: a goal, one put through the wrong net, or a penalty that did not go in. */
type GoalKind = 'scored' | 'own' | 'missed';
...
// Leaving the reason alone is the same as not giving one, so the list holds
// only the reasons themselves.
const reasonOptions = SUBSTITUTION_REASONS;

// An own goal is filed against the team that conceded it, so it counts on the
// opponent's scoreline. Mirrors what the API recomputes.
function scoresFor(type: EventType, eventTeamId: string, side: string, otherSide: string): boolean {
  if (type === 'goal') return eventTeamId === side;
  return type === 'own_goal' && eventTeamId === otherSide;
}

const asOption = (player: { id: string; name: string; jersey_number?: number | null }) => ({ label: `#${player.jersey_number ?? '–'} ${player.name}`, value: player.id });
...
export default function LiveScoringScreen() {
  ...
  const eventType: EventType = group === 'cards' ? cardType ?? 'yellow_card'
    : group === 'goal' ? ({ scored: 'goal', own: 'own_goal', missed: 'penalty_missed' } as const)[goalKind]
      : group;
  const awaitingCard = group === 'cards' && cardType === null;
  // A missed penalty is only worth logging once it says how it was missed, so
  // the outcome is picked first the same way a card is.
  const awaitingOutcome = eventType === 'penalty_missed' && outcome === null;
  ...
  const otherSideOf = (side: string) => side === matchQuery.data?.match.home_team_id ? matchQuery.data?.match.away_team_id ?? '' : matchQuery.data?.match.home_team_id ?? '';
  // The admin picks the side the goal counts for. An own goal is filed against
  // the side that put it in — who the scorer plays for — so the two swap on the
  // way to the API, and the scorer list follows the stored side.
  const eventTeamId = eventType === 'own_goal' ? otherSideOf(teamId) : teamId;
  ...
  useEffect(() => { if (eventTeamId && eligiblePlayers.every((player) => player.id !== playerId || player.team_id !== eventTeamId)) setPlayerId(''); }, [eventTeamId, eligiblePlayers, playerId]);
  ...
  const addEventMutation = useMutation({
    mutationFn: () => api.createEvent(id, { type: eventType, minute: minute ? Number(minute) : null, team_id: eventTeamId, player_id: playerId || null, secondary_player_id: eventType === 'substitution' || eventType === 'goal' ? offPlayerId || null : null, is_penalty: eventType === 'goal' && isPenalty, substitution_reason: eventType === 'substitution' ? reason : null, penalty_outcome: eventType === 'penalty_missed' ? outcome : null, client_operation_id: `${Date.now()}-${Math.random().toString(36).slice(2)}` }),
    onMutate: async () => { await client.cancelQueries({ queryKey: ['live-match', id] }); const previous = client.getQueryData<LiveMatchSnapshot>(['live-match', id]); if (previous) { const optimistic: MatchEvent = { id: `pending-${Date.now()}`, match_id: id, is_penalty: eventType === 'goal' && isPenalty, substitution_reason: eventType === 'substitution' ? reason : null, penalty_outcome: eventType === 'penalty_missed' ? outcome : null, type: eventType, minute: minute ? Number(minute) : null, team_id: eventTeamId, player_id: playerId || null, secondary_player_id: null, related_event_id: null, notes: null, client_operation_id: 'pending', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; client.setQueryData<LiveMatchSnapshot>(['live-match', id], { ...previous, events: [...previous.events, optimistic], match: { ...previous.match, home_score: previous.match.home_score + (scoresFor(eventType, eventTeamId, previous.match.home_team_id, previous.match.away_team_id) ? 1 : 0), away_score: previous.match.away_score + (scoresFor(eventType, eventTeamId, previous.match.away_team_id, previous.match.home_team_id) ? 1 : 0) } }); } return { previous }; },
    ...
    onSuccess: async () => { await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); setMinuteEdited(false); setIsPenalty(false); setOffPlayerId(''); setReason(null); setOutcome(null); await invalidateAfterWrite(client, 'event'); },
  });
  ...
  /**
   * Who can come on and who can come off, from the same spells the minutes are
   * counted from rather than a second tally of the same thing.
   *
   * A player is on the pitch if their spell has not ended, on the bench if they
   * are named but have no spell yet, and neither once they have been taken off.
   * A side with no lineup — the opponent, usually — falls back to its whole
   * squad, since there is nothing recorded to narrow it down to.
   */
  const substitution = useMemo(() => {
    const named = matchQuery.data?.lineup.filter((entry) => entry.team_id === eventTeamId) ?? [];
    const squad = eligiblePlayers.filter((player) => player.team_id === eventTeamId);
    if (!named.length) return { onPitch: squad, bench: squad };
    const spellOf = (playerId: string) => spells.find((spell) => spell.playerId === playerId);
    const holding = (ids: Set<string>) => squad.filter((player) => ids.has(player.id));
    return {
      onPitch: holding(new Set(named.filter((entry) => spellOf(entry.player_id)?.offAt === null).map((entry) => entry.player_id))),
      bench: holding(new Set(named.filter((entry) => !spellOf(entry.player_id)).map((entry) => entry.player_id))),
    };
  }, [matchQuery.data, eventTeamId, eligiblePlayers, spells]);
  // A pick that stops being eligible — the team changed, or the player has just
  // been substituted — is dropped rather than left to be refused on save.
  useEffect(() => {
    if (group !== 'substitution') return;
    if (playerId && !substitution.bench.some((player) => player.id === playerId)) setPlayerId('');
    if (offPlayerId && !substitution.onPitch.some((player) => player.id === offPlayerId)) setOffPlayerId('');
  }, [group, offPlayerId, playerId, substitution]);
  // The API only accepts a player with an appearance recorded, so the picker
  // offers exactly the players who took the pitch.
  const appearedPlayers = useMemo(
    () => spells.map((spell) => eligiblePlayers.find((player) => player.id === spell.playerId)).filter((player) => player !== undefined),
    [spells, eligiblePlayers],
  );
  const motmMutation = useMutation({
    mutationFn: (playerId: string | null) => api.setManOfTheMatch(id, playerId),
    onError: (error) => showMessage('Award not saved', (error as ApiError).message),
    onSuccess: async (updated) => {
      client.setQueryData<LiveMatchSnapshot>(['live-match', id], (current) => current ? { ...current, match: updated, revision: updated.revision } : current);
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      await invalidateAfterWrite(client, 'award');
    },
  });
  ...
  return <Screen action={<CloseButton />} title="Live scoring">
    {matchQuery.isLoading ? <LoadingState /> : matchQuery.isError || !match ? <ErrorState message={(matchQuery.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => matchQuery.refetch()} /> : <>
      ...
      {match.status !== 'scheduled' ? <View style={styles.panel}><Text style={styles.heading}>Add event</Text><View style={styles.eventTypes}>{eventGroups.map((option) => <Pressable accessibilityRole="button" accessibilityState={{ selected: group === option.value }} key={option.value} onPress={() => { setGroup(option.value); if (option.value === 'cards') setCardType(null); if (option.value !== 'substitution') setReason(null); if (option.value !== 'goal') { setGoalKind('scored'); setOutcome(null); } }} style={({ pressed }) => [styles.eventType, group === option.value && styles.eventTypeActive, pressed && styles.pressed]}>{option.icon ? <Ionicons color={group === option.value ? colors.onAccent : colors.textSecondary} name={option.icon} size={20} /> : <CardsIcon size={20} />}<Text style={[styles.eventLabel, group === option.value && styles.eventLabelActive]}>{option.label}</Text></Pressable>)}</View>
        ...
        {group === 'goal' ? <View style={styles.goalKinds}>
          {/* A goal that beat the keeper the wrong way, and a penalty that beat
              nobody, are both things that happen to a goal attempt, so they are
              logged from here rather than from a tab of their own. */}
          <Pressable accessibilityLabel="Own goal" accessibilityRole="checkbox" accessibilityState={{ checked: goalKind === 'own' }} onPress={() => { setGoalKind((current) => current === 'own' ? 'scored' : 'own'); setIsPenalty(false); setOutcome(null); setOffPlayerId(''); }} style={({ pressed }) => [styles.penaltyRow, goalKind === 'own' && styles.penaltyRowActive, pressed && styles.pressed]}>
            <View style={[styles.penaltyBox, goalKind === 'own' && styles.penaltyBoxActive]}>{goalKind === 'own' ? <Ionicons color={colors.onAccent} name="checkmark" size={16} /> : null}</View>
            <View style={styles.penaltyCopy}><Text style={styles.penaltyLabel}>Own goal</Text><Text style={styles.penaltyHint}>Counts for the side you pick below</Text></View>
          </Pressable>
          <Pressable accessibilityLabel="Penalty missed" accessibilityRole="checkbox" accessibilityState={{ checked: goalKind === 'missed' }} onPress={() => { setGoalKind((current) => current === 'missed' ? 'scored' : 'missed'); setIsPenalty(false); setOutcome(null); setOffPlayerId(''); }} style={({ pressed }) => [styles.penaltyRow, goalKind === 'missed' && styles.penaltyRowActive, pressed && styles.pressed]}>
            <View style={[styles.penaltyBox, goalKind === 'missed' && styles.penaltyBoxActive]}>{goalKind === 'missed' ? <Ionicons color={colors.onAccent} name="checkmark" size={16} /> : null}</View>
            <View style={styles.penaltyCopy}><Text style={styles.penaltyLabel}>Penalty missed</Text><Text style={styles.penaltyHint}>Logged without touching the score</Text></View>
          </Pressable>
        </View> : null}
        {eventType === 'penalty_missed' ? <View style={styles.cardChoices}>{PENALTY_OUTCOMES.map((choice) => <Pressable accessibilityRole="button" accessibilityState={{ selected: outcome === choice.value }} key={choice.value} onPress={() => setOutcome(choice.value)} style={({ pressed }) => [styles.outcomeChoice, outcome === choice.value && styles.cardChoiceActive, pressed && styles.pressed]}><Ionicons color={outcome === choice.value ? colors.accentSoft : colors.textSecondary} name={choice.value === 'saved' ? 'hand-left-outline' : 'arrow-redo-outline'} size={26} /><Text style={[styles.eventLabel, outcome === choice.value && styles.cardChoiceLabelActive]}>{choice.label}</Text></Pressable>)}</View> : null}
        {awaitingCard ? <Text style={styles.cardHint}>Pick a card to log it.</Text> : awaitingOutcome ? <Text style={styles.cardHint}>Say how the penalty was missed to log it.</Text> : <>
        <ChoiceField label={eventType === 'own_goal' ? 'Counts for' : 'Team'} onChange={setTeamId} options={[{ label: match.home_team?.name ?? 'Home', value: match.home_team_id }, { label: match.away_team?.name ?? 'Away', value: match.away_team_id }]} value={teamId} /><ChoiceField placeholder={eventType === 'substitution' ? (substitution.bench.length ? 'Choose a substitute' : 'Nobody is on the bench') : 'Choose a player'} label={eventType === 'substitution' ? 'Coming on' : eventType === 'goal' ? 'Scorer' : eventType === 'own_goal' ? `Put through their own net (${(eventTeamId === match.home_team_id ? match.home_team?.name : match.away_team?.name) ?? 'the other side'})` : eventType === 'penalty_missed' ? 'Taker' : 'Player (optional)'} onChange={setPlayerId} options={eventType === 'substitution' ? substitution.bench.map(asOption) : [{ label: 'No player selected', value: '' }, ...eligiblePlayers.filter((player) => player.team_id === eventTeamId).map(asOption)]} value={playerId} />{eventType === 'substitution' || eventType === 'goal' ? <ChoiceField placeholder={eventType === 'substitution' ? (substitution.onPitch.length ? 'Choose a player on the pitch' : 'Nobody is on the pitch') : 'Choose a player'} label={eventType === 'goal' ? 'Assisted by (optional)' : 'Coming off'} onChange={setOffPlayerId} options={eventType === 'substitution' ? substitution.onPitch.filter((player) => player.id !== playerId).map(asOption) : [{ label: 'No assist', value: '' }, ...eligiblePlayers.filter((player) => player.team_id === eventTeamId && player.id !== playerId).map(asOption)]} value={offPlayerId} /> : null}{eventType === 'substitution' ? <ChoiceField label="Reason (optional)" onChange={(value) => setReason((value || null) as SubstitutionReason | null)} options={reasonOptions} placeholder="No reason given" value={reason ?? ''} /> : null}{eventType === 'goal' ? <Pressable accessibilityLabel="Scored from a penalty" accessibilityRole="checkbox" accessibilityState={{ checked: isPenalty }} onPress={() => setIsPenalty((current) => !current)} style={({ pressed }) => [styles.penaltyRow, isPenalty && styles.penaltyRowActive, pressed && styles.pressed]}><View style={[styles.penaltyBox, isPenalty && styles.penaltyBoxActive]}>{isPenalty ? <Ionicons color={colors.onAccent} name="checkmark" size={16} /> : null}</View><View style={styles.penaltyCopy}><Text style={styles.penaltyLabel}>Scored from a penalty</Text><Text style={styles.penaltyHint}>Still counts on the scoreline</Text></View></Pressable> : null}<View><Text style={styles.inputLabel}>Minute (optional)</Text><TextInput accessibilityLabel="Event minute" keyboardType="number-pad" maxLength={3} onChangeText={setMinute} placeholder="67" placeholderTextColor={colors.textMuted} style={styles.input} value={minute} /></View><AppButton disabled={!teamId || addEventMutation.isPending} label={addEventMutation.isPending ? 'Saving event' : `Add ${selectedCard?.label ?? (eventType === 'own_goal' ? 'own goal' : eventType === 'penalty_missed' ? 'missed penalty' : eventGroups.find((option) => option.value === group)?.label)}`} onPress={() => addEventMutation.mutate()} tint={selectedCard ? cardTint(selectedCard.value) : undefined} />
        </>}
      </View> : null}
      ...
      {match.status === 'finished' ? <View style={styles.panel}>
        <Text style={styles.heading}>Man of the match</Text>
        <Text style={styles.help}>Only players marked as having appeared can be picked. Save minutes first if the list is empty.</Text>
        <ChoiceField
          label="Player"
          onChange={(value) => motmMutation.mutate(value || null)}
          options={[{ label: 'No one picked', value: '' }, ...appearedPlayers.map((player) => ({ label: `#${player.jersey_number ?? '–'} ${player.name}`, value: player.id }))]}
          value={match.man_of_the_match_player_id ?? ''}
        />
      </View> : null}
