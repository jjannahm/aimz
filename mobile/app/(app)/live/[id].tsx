...
/** Stands in for a player id, since the other side has no player records here. */
const OPPONENT_AWARD = 'opponent';
...
// The reason is optional, so the empty value is offered first.
const reasonOptions = [{ label: 'No reason given', value: '' }, ...SUBSTITUTION_REASONS];
...
export default function LiveScoringScreen() {
  ...
  const motmMutation = useMutation({
    mutationFn: (choice: string | null) => api.setManOfTheMatch(id, choice === OPPONENT_AWARD ? null : choice, choice === OPPONENT_AWARD),
    ...
    {matchQuery.isLoading ? <LoadingState /> : matchQuery.isError || !match ? <ErrorState message={(matchQuery.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => matchQuery.refetch()} /> : <>
      ...
      {match.status === 'finished' ? <View style={styles.panel}>
        ...
        <ChoiceField
          ...
          options={[{ label: 'No one picked', value: '' }, ...appearedPlayers.map((player) => ({ label: `#${player.jersey_number ?? '–'} ${player.name}`, value: player.id })), { label: 'Opponent player', value: OPPONENT_AWARD }]}
          value={match.man_of_the_match_is_opponent ? OPPONENT_AWARD : match.man_of_the_match_player_id ?? ''}
        />
      </View> : null}
