...
const eventGroups: { label: string; value: EventGroup; icon?: keyof typeof Ionicons.glyphMap }[] = [
  ...
  { label: 'Own goal', value: 'own_goal', icon: 'football-outline' }, { label: 'Pen miss', value: 'penalty_missed', icon: 'close-circle-outline' },
];
...
export default function LiveScoringScreen() {
  ...
  const [teamId, setTeamId] = useState(''); const [playerId, setPlayerId] = useState(''); const [minute, setMinute] = useState(''); const [group, setGroup] = useState<EventGroup>('goal'); const [cardType, setCardType] = useState<CardType | null>(null); const [isPenalty, setIsPenalty] = useState(false); const [offPlayerId, setOffPlayerId] = useState(''); const [minuteEdited, setMinuteEdited] = useState(false); const [reason, setReason] = useState<SubstitutionReason | null>(null); const [outcome, setOutcome] = useState<PenaltyOutcome | null>(null);
  ...
  const awaitingOutcome = group === 'penalty_missed' && outcome === null;
  ...
  const addEventMutation = useMutation({
    mutationFn: () => api.createEvent(id, { type: eventType, minute: minute ? Number(minute) : null, team_id: teamId, player_id: playerId || null, secondary_player_id: eventType === 'substitution' || eventType === 'goal' ? offPlayerId || null : null, is_penalty: eventType === 'goal' && isPenalty, substitution_reason: eventType === 'substitution' ? reason : null, penalty_outcome: eventType === 'penalty_missed' ? outcome : null, client_operation_id: `${Date.now()}-${Math.random().toString(36).slice(2)}` }),
    onMutate: async () => { await client.cancelQueries({ queryKey: ['live-match', id] }); const previous = client.getQueryData<LiveMatchSnapshot>(['live-match', id]); if (previous) { const optimistic: MatchEvent = { id: `pending-${Date.now()}`, match_id: id, is_penalty: eventType === 'goal' && isPenalty, substitution_reason: eventType === 'substitution' ? reason : null, penalty_outcome: eventType === 'penalty_missed' ? outcome : null, type: eventType, minute: minute ? Number(minute) : null, team_id: teamId, player_id: playerId || null, secondary_player_id: null, related_event_id: null, notes: null, client_operation_id: 'pending', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; client.setQueryData<LiveMatchSnapshot>(['live-match', id], { ...previous, events: [...previous.events, optimistic], match: { ...previous.match, home_score: previous.match.home_score + (scoresFor(eventType, teamId, previous.match.home_team_id, previous.match.away_team_id) ? 1 : 0), away_score: previous.match.away_score + (scoresFor(eventType, teamId, previous.match.away_team_id, previous.match.home_team_id) ? 1 : 0) } }); } return { previous }; },
  ...
  return <Screen action={<CloseButton />} title="Live scoring">
    {matchQuery.isLoading ? <LoadingState /> : matchQuery.isError || !match ? <ErrorState message={(matchQuery.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => matchQuery.refetch()} /> : <>
      ...
      {match.status !== 'scheduled' ? <View style={styles.panel}><Text style={styles.heading}>Add event</Text><View style={styles.eventTypes}>{eventGroups.map((option) => <Pressable accessibilityRole="button" accessibilityState={{ selected: group === option.value }} key={option.value} onPress={() => { setGroup(option.value); if (option.value === 'cards') setCardType(null); if (option.value === 'penalty_missed') setOutcome(null); if (option.value !== 'substitution') setReason(null); }} style={({ pressed }) => [styles.eventType, group === option.value && styles.eventTypeActive, pressed && styles.pressed]}>{option.icon ? <Ionicons color={group === option.value ? colors.onAccent : colors.textSecondary} name={option.icon} size={20} /> : <CardsIcon size={20} />}<Text style={[styles.eventLabel, group === option.value && styles.eventLabelActive]}>{option.label}</Text></Pressable>)}</View>
