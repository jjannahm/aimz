...
export default function SettingsScreen() {
  ...
  const savePassword = password.handleSubmit(async ({ current, next }) => { try { await api.changePassword(current, next); password.reset(); Alert.alert('Password updated', 'Use your new password the next time you sign in.'); } catch (error) { password.setError('root', { message: (error as ApiError).message }); } });
