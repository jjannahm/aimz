...
import type { BadgeStyle, Competition, Match, MatchTimeStructure, Player, RegistrationInvite, Team } from '@/src/types/api';
...
function entityMeta(item: Entity) { if ('home_team_id' in item) return `${item.status.toUpperCase()} · ${formatEgyptDateTime(item.kickoff_datetime)}`; if ('position' in item) return `${item.position} · #${item.jersey_number ?? '–'}`; if ('use_count' in item) return `${item.is_active ? 'Active' : 'Revoked'} · ${item.use_count} uses`; if ('is_aimz' in item) return `${item.is_active ? '' : 'Archived · '}${item.squad_code ?? (item.is_aimz ? [item.age_group, item.squad_code, item.season].filter(Boolean).join(' · ') || 'AIMZ squad' : 'Opponent club')}`; return `${item.type} · ${item.season}`; }
