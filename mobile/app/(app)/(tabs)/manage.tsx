...
import type { Competition, InviteKind, Match, MatchTimeStructure, Player, RegistrationInvite, Team } from '@/src/types/api';
...
export default function ManageScreen() {
  ...
  // Eight sections in a four-across grid rather than a strip that scrolled the
  // last of them out of sight. Each cell is exactly a quarter, so the columns
  // line up down both rows.
  const resourceChips = <View style={styles.chips}>{resources.map((item) => <View key={item.value} style={styles.chipCell}>
    <Pressable accessibilityRole="tab" accessibilityState={{ selected: resource === item.value }} onPress={() => switchResource(item.value)} style={({ pressed }) => [styles.chip, resource === item.value && styles.chipActive, pressed && styles.pressed]}>
      <Text numberOfLines={2} style={[styles.chipText, resource === item.value && styles.chipTextActive]}>{item.label}</Text>
    </Pressable>
  </View>)}</View>;
