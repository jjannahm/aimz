...
const schema = z.object({ name: z.string(), code: z.string(), ageGroup: z.string(), season: z.string(), isAimz: z.string(), type: z.string(), teamId: z.string(), position: z.string(), jersey: z.string(), competitionId: z.string(), homeTeamId: z.string(), awayTeamId: z.string(), kickoff: z.string(), venue: z.string(), status: z.string(), halfLength: z.string(), numHalves: z.string(), halfTimeBreak: z.string(), label: z.string() });
type Values = z.infer<typeof schema>;
const defaults: Values = { name: '', code: '', ageGroup: '', season: '', isAimz: 'true', type: 'league', teamId: '', position: '', jersey: '', competitionId: '', homeTeamId: '', awayTeamId: '', kickoff: new Date().toISOString(), venue: '', status: 'scheduled', halfLength: '45', numHalves: '2', halfTimeBreak: '15', label: '' };

/** Parses the three period inputs, or null when any is not a whole number in range. */
function readTimeStructure(values: Pick<Values, 'halfLength' | 'numHalves' | 'halfTimeBreak'>): MatchTimeStructure | null {
  ...
  return { half_length_minutes, num_halves, half_time_break_minutes };
}

function MatchLengthSummary({ control }: { control: Control<Values> }) {
  const [halfLength, numHalves, halfTimeBreak] = useWatch({ control, name: ['halfLength', 'numHalves', 'halfTimeBreak'] });
  const structure = readTimeStructure({ halfLength, numHalves, halfTimeBreak });
