...
import { AccountsManager, AnnouncementsManager, ScheduleManager } from '@/src/components/manage/HubManagers';
...
export default function ManageScreen() {
  ...
  const resourceChips = <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips} style={styles.chipBar}>{resources.map((item) => <Pressable accessibilityRole="tab" accessibilityState={{ selected: resource === item.value }} key={item.value} onPress={() => switchResource(item.value)} style={({ pressed }) => [styles.chip, resource === item.value && styles.chipActive, pressed && styles.pressed]}><Text style={[styles.chipText, resource === item.value && styles.chipTextActive]}>{item.label}</Text></Pressable>)}</ScrollView>;
  const aimzTeams = teams.data?.items.filter((team) => team.is_aimz) ?? [];
  if (resource === 'schedule' || resource === 'announcements' || resource === 'accounts') return <Screen scrollRef={pageRef} title="Manage academy">
    {resourceChips}
    {resource === 'schedule' ? <ScheduleManager teams={aimzTeams} /> : resource === 'announcements' ? <AnnouncementsManager teams={aimzTeams} /> : <AccountsManager players={players.data?.items ?? []} />}
  </Screen>;
  ...
  const save = form.handleSubmit(async (values) => {
    ...
    try {
      if (resource === 'teams' || resource === 'opponents') {
        ...
        const payload = { name: values.name.trim(), squad_code: values.code.trim() || null, age_group: values.ageGroup.trim() || null, season: values.season.trim() || null, is_aimz: resource === 'teams', is_active: true, competition_id: values.teamCompetitionId || null, competition_group_id: values.teamGroupId || null, coach: values.coach.trim() || null, assistant_coach: values.assistantCoach.trim() || null, logo_key: editing && 'logo_key' in editing ? editing.logo_key : null };
        ...
        await api.createInvite({ label: values.label.trim(), code: values.code.trim(), player_id: values.playerLinkId || null });
      }
...
function ResourceFields({ control, resource, setValue, teams, competitions, editingId, players }: { control: any; errors: any; resource: LegacyResource; setValue: any; teams: Team[]; competitions: Competition[]; editingId: string | null; players: Player[] }) {
  ...
  return <><Controller control={control} name="label" render={({ field }) => <FormField label="Invite label" onChangeText={field.onChange} placeholder="Autumn intake" value={field.value} />} /><Controller control={control} name="code" render={({ field }) => <FormField autoCapitalize="characters" label="Invite code" onChangeText={field.onChange} placeholder="AIMZ-2026" value={field.value} />} /><Controller control={control} name="playerLinkId" render={({ field }) => <ChoiceField label="Player (optional)" onChange={field.onChange} options={[{ label: 'General invitation', value: '' }, ...players.map((player) => ({ label: player.name, value: player.id }))]} value={field.value} />} /><Text style={styles.pickerNote}>A personal invitation can be used once. The new account is linked to this roster record automatically.</Text></>;
}
