...
const schema = z.object({ name: z.string(), code: z.string(), ageGroup: z.string(), season: z.string(), type: z.string(), teamId: z.string(), position: z.string(), jersey: z.string(), competitionId: z.string(), homeTeamId: z.string(), awayTeamId: z.string(), kickoff: z.string(), venue: z.string(), status: z.string(), halfLength: z.string(), numHalves: z.string(), halfTimeBreak: z.string(), coach: z.string(), assistantCoach: z.string(), hasExtraTime: z.string(), extraTimeLength: z.string(), label: z.string() });
type Values = z.infer<typeof schema>;
const defaults: Values = { name: '', code: '', ageGroup: '', season: '', type: 'league', teamId: '', position: '', jersey: '', competitionId: '', homeTeamId: '', awayTeamId: '', kickoff: new Date().toISOString(), venue: '', status: 'scheduled', halfLength: '45', numHalves: '2', halfTimeBreak: '15', coach: '', assistantCoach: '', hasExtraTime: 'false', extraTimeLength: '15', label: '' };
...
export default function ManageScreen() {
  ...
  const save = form.handleSubmit(async (values) => {
    ...
    try {
      if (resource === 'teams' || resource === 'opponents') {
        ...
        const payload = { name: values.name.trim(), squad_code: values.code.trim() || null, age_group: values.ageGroup.trim() || null, season: values.season.trim() || null, is_aimz: resource === 'teams', is_active: true, coach: values.coach.trim() || null, assistant_coach: values.assistantCoach.trim() || null, logo_key: editing && 'logo_key' in editing ? editing.logo_key : null };
  ...
  const beginEdit = (item: Entity) => {
    ...
    if ((resource === 'teams' || resource === 'opponents') && 'is_aimz' in item) form.reset({ ...defaults, name: item.name, code: item.squad_code ?? '', ageGroup: item.age_group ?? '', season: item.season ?? '', coach: item.coach ?? '', assistantCoach: item.assistant_coach ?? '' });
...
function entityMeta(item: Entity) { if ('home_team_id' in item) return `${item.status.toUpperCase()} · ${new Date(item.kickoff_datetime).toLocaleString('en-EG')}`; if ('position' in item) return `${item.position} · #${item.jersey_number ?? '–'}`; if ('use_count' in item) return `${item.is_active ? 'Active' : 'Revoked'} · ${item.use_count} uses`; if ('is_aimz' in item) return item.squad_code ?? (item.is_aimz ? [item.age_group, item.squad_code, item.season].filter(Boolean).join(' · ') || 'AIMZ squad' : 'Opponent club'); return `${item.type} · ${item.season}`; }
