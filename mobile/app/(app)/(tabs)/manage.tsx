...
import { EXTRA_TIME_PERIODS, isKnockout, KNOCKOUT_TEAM_COUNTS, totalMatchMinutes } from '@/src/types/api';
...
function KnockoutSize({ control }: { control: Control<Values> }) {
  ...
  return <>
    ...
    <Text style={styles.pickerNote}>Groups and an empty bracket are drawn up as soon as the competition is saved. The size is fixed once teams are entered.</Text>
  </>;
}
...
function ResourceFields({ control, errors, resource, setValue, teams, competitions }: { control: any; errors: any; resource: Resource; setValue: any; teams: Team[]; competitions: Competition[] }) {
  ...
  if (resource === 'competitions') return <><Controller control={control} name="name" render={({ field }) => <FormField label="Competition name" onChangeText={field.onChange} value={field.value} />} /><Controller control={control} name="season" render={({ field }) => <FormField label="Season" onChangeText={field.onChange} placeholder="2026/27" value={field.value} />} /><Controller control={control} name="type" render={({ field }) => <ChoiceField label="Format" onChange={field.onChange} options={[{ label: 'League', value: 'league' }, { label: 'Knockout (groups and a bracket)', value: 'tournament' }, { label: 'Friendly', value: 'friendly' }]} value={field.value} />} /><KnockoutSize control={control} /></>;
