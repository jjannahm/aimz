...
export default function ManageScreen() {
  ...
  // Eight sections in a grid rather than a strip that scrolled the last of them
  // out of sight. Three to a row, not four: a quarter of a phone is narrower
  // than "Announcements" at any legible size, and a third is not. The last row
  // holds the remaining two, centred under the rest.
  const resourceChips = <View style={styles.chips}>{resources.map((item) => <View key={item.value} style={styles.chipCell}>
    <Pressable accessibilityRole="tab" accessibilityState={{ selected: resource === item.value }} onPress={() => switchResource(item.value)} style={({ pressed }) => [styles.chip, resource === item.value && styles.chipActive, pressed && styles.pressed]}>
      <Text numberOfLines={1} style={[styles.chipText, resource === item.value && styles.chipTextActive]}>{item.label}</Text>
    </Pressable>
  </View>)}</View>;
