...
import type { Competition, Match, Player, RegistrationInvite, Team } from '@/src/types/api';
...
const schema = z.object({ name: z.string(), code: z.string(), ageGroup: z.string(), season: z.string(), isAimz: z.string(), type: z.string(), teamId: z.string(), position: z.string(), jersey: z.string(), competitionId: z.string(), homeTeamId: z.string(), awayTeamId: z.string(), kickoff: z.string(), venue: z.string(), status: z.string(), label: z.string() });
type Values = z.infer<typeof schema>;
const defaults: Values = { name: '', code: '', ageGroup: '', season: '', isAimz: 'true', type: 'league', teamId: '', position: '', jersey: '', competitionId: '', homeTeamId: '', awayTeamId: '', kickoff: new Date().toISOString(), venue: '', status: 'scheduled', label: '' };
...
export default function ManageScreen() {
  ...
  const save = form.handleSubmit(async (values) => {
    ...
    try {
      if (resource === 'teams') {
        ...
        const payload = { competition_id: values.competitionId, home_team_id: values.homeTeamId, away_team_id: values.awayTeamId, kickoff_datetime: new Date(values.kickoff).toISOString(), venue: values.venue.trim(), status: values.status as Match['status'] };
  ...
  const beginEdit = (item: Entity) => {
    ...
    if (resource === 'matches' && 'status' in item) form.reset({ ...defaults, competitionId: item.competition_id, homeTeamId: item.home_team_id, awayTeamId: item.away_team_id, kickoff: item.kickoff_datetime, venue: item.venue, status: item.status });
  };
