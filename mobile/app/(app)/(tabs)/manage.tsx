...
const schema = z.object({ name: z.string(), code: z.string(), ageGroup: z.string(), season: z.string(), isAimz: z.string(), type: z.string(), teamId: z.string(), position: z.string(), jersey: z.string(), competitionId: z.string(), homeTeamId: z.string(), awayTeamId: z.string(), kickoff: z.string(), venue: z.string(), status: z.string(), halfLength: z.string(), numHalves: z.string(), halfTimeBreak: z.string(), coach: z.string(), assistantCoach: z.string(), hasExtraTime: z.string(), extraTimeLength: z.string(), label: z.string() });
type Values = z.infer<typeof schema>;
const defaults: Values = { name: '', code: '', ageGroup: '', season: '', isAimz: 'true', type: 'league', teamId: '', position: '', jersey: '', competitionId: '', homeTeamId: '', awayTeamId: '', kickoff: new Date().toISOString(), venue: '', status: 'scheduled', halfLength: '45', numHalves: '2', halfTimeBreak: '15', coach: '', assistantCoach: '', hasExtraTime: 'false', extraTimeLength: '15', label: '' };
  ...
  const save = form.handleSubmit(async (values) => {
    ...
    try {
      if (resource === 'teams') {
        ...
        const payload = { name: values.name.trim(), squad_code: values.code.trim() || null, age_group: values.ageGroup.trim() || null, season: values.season.trim() || null, is_aimz: values.isAimz === 'true', is_active: true, coach: values.coach.trim() || null, assistant_coach: values.assistantCoach.trim() || null, logo_key: editing && 'logo_key' in editing ? editing.logo_key : null };
