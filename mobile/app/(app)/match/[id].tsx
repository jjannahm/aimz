      ...
      {!opponentOnly || query.data.lineup.length ? <View style={styles.section}>
        ...
        {(() => {
          ...
          const row = (entry: typeof query.data.lineup[number]) => {
            ...
            const name = playerNames.get(entry.player_id) ?? 'Player';
            // The same tap as a shirt on the pitch above, from the list below it.
            return <Pressable accessibilityLabel={`${name}, open stats`} accessibilityRole="button" key={entry.id} onPress={() => router.push(`/player/${entry.player_id}`)} style={({ pressed }) => [styles.lineupRow, pressed && styles.pressed]}>
              <JerseyIcon number={entry.jersey_number} size={34} />
              <View style={styles.lineupCopy}><Text style={styles.lineupName}>{name}{entry.is_captain ? ' (C)' : ''}</Text><Text style={styles.lineupMeta}>{describe(entry)}</Text></View>
