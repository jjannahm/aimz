    ...
    {query.isLoading ? <LoadingState label="Loading match" /> : query.isError || !query.data ? <ErrorState message={(query.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => query.refetch()} /> : <>
      ...
      <View style={styles.section}>
        ...
        {(() => {
          ...
          const squad = [query.data.match.home_team, query.data.match.away_team].find((team) => team?.is_aimz) ?? null;
          const row = (entry: typeof query.data.lineup[number]) => <View key={entry.id} style={styles.lineupRow}>
          ...
          return <>
            {squad?.coach || squad?.assistant_coach ? <View style={styles.staffRow}>
              <View style={styles.staff}><Text style={styles.staffLabel}>Coach</Text><Text numberOfLines={1} style={styles.staffName}>{squad.coach ?? 'Not set'}</Text></View>
              <View style={styles.staff}><Text style={styles.staffLabel}>Assistant coach</Text><Text numberOfLines={1} style={styles.staffName}>{squad.assistant_coach ?? 'Not set'}</Text></View>
            </View> : null}
            {query.data.match.formation && starters.length ? <FormationPitch formation={query.data.match.formation} starters={starters.map(asPlayer)} /> : null}
