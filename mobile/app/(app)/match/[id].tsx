  ...
  return <Screen action={<Pressable accessibilityLabel="Go back" accessibilityRole="button" hitSlop={10} onPress={() => router.back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><Ionicons color={theme.colors.textPrimary} name="chevron-back" size={24} /></Pressable>} eyebrow="Match detail" title="Game centre">
    {query.isLoading ? <LoadingState label="Loading match" /> : query.isError || !query.data ? <ErrorState message={(query.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => query.refetch()} /> : <>
      ...
      <View style={styles.section}><Text style={styles.sectionTitle}>Timeline</Text>{query.data.events.length === 0 ? <Text style={styles.empty}>Match events will appear here.</Text> : query.data.events.map((event) => {
        ...
        // than arriving as a second row.
        const assist = event.type === 'goal' && event.secondary_player_id ? playerNames.get(event.secondary_player_id) : null;
        const swappedOn = event.type === 'substitution' && event.secondary_player_id ? playerNames.get(event.secondary_player_id) : null;
        return <View key={event.id} style={styles.event}>
          <View style={styles.eventIcon}><Ionicons color={event.type === 'red_card' ? theme.colors.error : event.type === 'yellow_card' ? theme.colors.warning : theme.colors.lightBlue} name={event.type === 'substitution' ? 'swap-horizontal' : event.type.includes('card') ? 'square' : 'football'} size={18} /></View>
          <View style={styles.eventCopy}>
            ...
            {swappedOn ? <Text style={styles.eventAssist}>Off: {swappedOn}</Text> : null}
            {event.type !== 'goal' ? <Text style={styles.eventPlayer}>{eventLabel[event.type]}</Text> : null}
          </View>
