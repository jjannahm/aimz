  ...
  return <Screen action={<CloseButton />} eyebrow="Match detail" title="Game centre">
    {query.isLoading ? <LoadingState label="Loading match" /> : query.isError || !query.data ? <ErrorState message={(query.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => query.refetch()} /> : <>
      ...
      <View style={styles.section}><Text style={styles.sectionTitle}>Timeline</Text>{query.data.events.length === 0 ? <Text style={styles.empty}>Match events will appear here.</Text> : query.data.events.map((event) => {
        ...
        return <View key={event.id} style={styles.event}>
          <View style={styles.eventIcon}><Ionicons color={event.type === 'red_card' ? colors.error : event.type === 'yellow_card' ? colors.warning : colors.accentSoft} name={event.type === 'substitution' ? 'swap-horizontal' : event.type.includes('card') ? 'square' : 'football'} size={18} /></View>
          <View style={styles.eventCopy}>
            <Text style={styles.eventTitle}>{lead}{event.type === 'goal' && event.is_penalty ? ' (pen)' : ''}</Text>
            {assist ? <Text style={styles.eventAssist}>Assist: {assist}</Text> : null}
            {cameOff ? <Text style={styles.eventAssist}>Off: {cameOff}</Text> : null}
            {event.type === 'goal' ? null : <Text style={styles.eventPlayer}>{eventLabel[event.type]}</Text>}
          </View>
          <Text style={styles.minute}>{event.minute == null ? 'FT' : `${event.minute}'`}</Text>
        </View>;
      })}</View>
      {(() => {
