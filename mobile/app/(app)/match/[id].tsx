    ...
    {query.isLoading ? <LoadingState label="Loading match" /> : query.isError || !query.data ? <ErrorState message={(query.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => query.refetch()} /> : <>
      ...
      <View style={styles.timeline}><Text style={styles.timelineTitle}>Timeline</Text>{query.data.events.length === 0 ? <Text style={[styles.empty, styles.timelineEmpty]}>Match events will appear here.</Text> : query.data.events.map((event) => {
        ...
        return <View key={event.id} style={[styles.event, styles.eventCompact]}>
          <View style={[styles.eventIcon, styles.eventIconCompact]}><Ionicons color={event.type === 'red_card' ? colors.error : event.type === 'yellow_card' ? colors.warning : colors.accentSoft} name={event.type === 'substitution' ? 'swap-horizontal' : event.type.includes('card') ? 'square' : 'football'} size={15} /></View>
          <View style={styles.eventCopy}>
            ...
            {event.type === 'goal' ? null : <Text style={[styles.eventPlayer, styles.compactSubText]}>{eventLabel[event.type]}</Text>}
          </View>
