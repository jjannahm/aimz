  ...
  return <Screen action={<Pressable accessibilityLabel="Go back" accessibilityRole="button" hitSlop={10} onPress={() => router.back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><Ionicons color={theme.colors.textPrimary} name="chevron-back" size={24} /></Pressable>} eyebrow="Match detail" title="Game centre">
    {query.isLoading ? <LoadingState label="Loading match" /> : query.isError || !query.data ? <ErrorState message={(query.error as ApiError)?.message ?? 'Match not found.'} onRetry={() => query.refetch()} /> : <>
      ...
      <View style={styles.section}>
        ...
        {(() => {
          const starters = query.data.lineup.filter((entry) => entry.is_starter);
          const subs = query.data.lineup.filter((entry) => !entry.is_starter);
          ...
          const describe = (entry: typeof query.data.lineup[number]) => [entry.position, entry.jersey_number == null ? null : `#${entry.jersey_number}`].filter(Boolean).join(' · ');
          ...
          return <>
            <Text style={styles.groupTitle}>Starting {starters.length}</Text>
            {starters.map((entry) => <View key={entry.id} style={styles.lineup}><Text style={styles.lineupName}>{playerNames.get(entry.player_id) ?? 'Player'}</Text><Text style={styles.lineupMeta}>{describe(entry) || 'Position not set'}</Text></View>)}
            <Text style={styles.groupTitle}>Substitutes {subs.length ? `(${subs.length})` : ''}</Text>
            {subs.length ? subs.map((entry) => <View key={entry.id} style={styles.lineup}><Text style={styles.lineupName}>{playerNames.get(entry.player_id) ?? 'Player'}</Text><Text style={styles.lineupMeta}>{describe(entry) || 'Position not set'}</Text></View>) : <Text style={styles.empty}>No substitutes named.</Text>}
