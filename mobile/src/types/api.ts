import type * as Contract from '@/src/types/contract';

export type UserRole = Contract.UserRole;
/**
 ...
 */
export type User = Contract.UserRead & { team_ids?: string[] | null };
/**
...
export type MatchStatus = Contract.MatchStatus;
export type MatchPhase = Contract.MatchPhase;
export type MatchPhaseAction = Contract.MatchPhaseAction;
type CompetitionType = Contract.CompetitionType;
export type EventType = Contract.EventType;
export type TokenResponse = Omit<Contract.TokenResponse, 'user'> & { user: User };
export type RegistrationInvite = Contract.InviteRead & { players?: { id: string; name: string }[]; code?: string; share_url?: string };
...
export type Competition = Omit<Contract.CompetitionRead, 'status' | 'completed_at'> & { status?: CompetitionStatus; completed_at?: string | null };
...
export type StandingRow = Omit<Contract.StandingRow, 'team' | 'form'> & { team: Team; form: FormResult[]; group?: CompetitionGroupRef | null };
...
export type PlayerMatchStat = Contract.PlayerMatchStatRead;
...
export type PlayerSeasonSummary = Omit<Contract.PlayerSeasonSummary, 'matches'> & GoalkeeperTotals & {
...
// The leaderboard row returned by the Worker.
...
export type Team = Contract.TeamRead;

export type Player = Contract.PlayerRead;
...
export type Match = Omit<Contract.MatchRead, 'home_team' | 'away_team' | 'competition' | 'lineup_format' | 'man_of_the_match_is_opponent'> & {
  home_team: Team | null;
  away_team: Team | null;
  competition: Competition | null;
