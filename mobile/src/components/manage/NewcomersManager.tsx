...
function Detail({ item, loading, teams, onBack, onChanged }: { item?: Newcomer; loading: boolean; teams: Team[]; onBack: () => void; onChanged: () => Promise<void> }) {
  ...
  // Two per row, and the free-text three across the whole width — an address
  // or a list of medications in a half-column wraps to four lines and undoes
  // the point of the grid. `wide` is what says which.
  const facts: { label: string; value: string | null; wide?: boolean }[] = [
    { label: 'Mobile', value: item.mobile }, { label: 'WhatsApp', value: item.whatsapp_mobile },
    { label: 'Email', value: item.email, wide: true },
    { label: 'Birth date', value: item.date_of_birth }, { label: 'Nationality', value: item.nationality },
    { label: 'Father', value: `${item.father_name} · ${item.father_mobile}`, wide: true },
    { label: 'Mother', value: `${item.mother_name} · ${item.mother_mobile}`, wide: true },
    { label: 'Previous academy', value: item.previous_academy }, { label: 'School / university', value: item.school_university },
    { label: 'Address', value: item.address, wide: true },
    { label: 'Medical concerns', value: item.medical_concerns, wide: true },
    { label: 'Medications', value: item.medications, wide: true },
  ];
  return <View style={styles.stack}><AppButton compact label="Back to newcomers" onPress={onBack} variant="ghost" /><Text style={styles.title}>{item.full_name}</Text>
    {/* Folded away by default: what an application is opened to do is move it
        along, and twelve rows of what the form already said stood between the
        name and the first control. */}
    <CollapsibleSection count={facts.length} size="group" title="Personal details">
      <View style={styles.facts}>{facts.map((fact) => <View key={fact.label} style={[styles.fact, fact.wide && styles.factWide]}><Text style={styles.factLabel}>{fact.label}</Text><Text selectable style={[styles.factValue, !fact.value && styles.factEmpty]}>{fact.value || '—'}</Text></View>)}</View>
    </CollapsibleSection>
